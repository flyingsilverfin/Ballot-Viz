...
documents.sort(key=lambda s: datetime.strptime(s.updated[:10], '%Y-%m-%d'), reverse=True)
doc = documents[0]
...
class RoomUpdater:
	...
	#2017
	#https://docs.google.com/spreadsheets/d/1opFLlCjuyFTCAs5gGr1RRMe1DaLvM3IuunB6dywUKVg/pub?output=csv
	...
	def updateBallotDocument(self):
		...
		for line in csv:
			line = line.strip()
			row = self.splitByUnquotedDelimiter(line, ",")
			#row = line.split(",")
			#this is a big ugly...
	...
	def splitByUnquotedDelimiter(self, line, delimiter):
		arr = [""]
		quoted = False
		for char in line:
